...
def planner_agent(state: AgentState) -> dict:
    ...
    prompt = f"""
You are the Master Planner Agent in a multi-agent AI system.
...
YOUR JOB:
Decide the best sequence of agents to complete this task end-to-end.

STRICT RULES:
1. Always include: research, tool, code, review, critic, memory, summarizer
2. "memory" must come directly before "summarizer"
3. "summarizer" must be LAST — always
4. Do NOT include "planner" in the plan
5. output_data must be a JSON ARRAY of agent name strings

{enforce_output_schema()}
Example of correct output_data: ["research", "tool", "code", "review", "critic", "memory", "summarizer"]
"""

    parsed = invoke_with_json_retry(llm, prompt, max_retries=4, fallback_key="output_data")
    ...
    # Ensure memory is before summarizer
    if "memory" not in clean_plan:
        clean_plan.insert(-1, "memory")
    elif clean_plan[-2] != "memory":
        clean_plan = [s for s in clean_plan if s != "memory"]
        clean_plan.insert(-1, "memory")

    # If planner returned garbage, use the safe default
    if len(clean_plan) < 3:
        print("[Planner] Plan too short — using default plan.")
        clean_plan = DEFAULT_PLAN
